import { IKeycloakRefreshTokenApiResponse } from '@/app/api/auth/keycloakRefreshToken';
      ...
      if (error.response?.status === 401 && requestsAttempts < maximumRequestsAttempts) {
        if ((error.response?.data as { errorCode: string })?.errorCode === 'token.invalid') {
          ...
          if (!isRefreshingToken) {
            ...
            axios
              .post<IKeycloakRefreshTokenApiResponse>('/api/auth/keycloakRefreshToken', {
              ...
              .then((refreshTokenResponse) => {
                ...
                failedRequestsQueue.forEach((request) => request.onSuccess(refreshTokenResponse.data.access_token));
                failedRequestsQueue = [];
              })
