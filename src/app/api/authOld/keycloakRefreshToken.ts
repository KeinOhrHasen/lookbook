...
async function keycloakRefreshToken(request: NextApiRequest, response: NextApiResponse) {
  if (request.method === 'POST') {
    try {
      ...
      keycloakParamsToRefreshToken.append('client_id', process.env.KEYCLOAK_CLIENT_ID!);
      keycloakParamsToRefreshToken.append('client_secret', process.env.KEYCLOAK_CLIENT_SECRET!);
      keycloakParamsToRefreshToken.append('grant_type', ['refresh_token'].toString());
      ...
      if (error instanceof AxiosError) {
        return response.status(error.response?.status || 401).json(error.response?.data || {});
      }
